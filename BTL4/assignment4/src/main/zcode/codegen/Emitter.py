...
class Emitter():
    ...
    *    @param typ the type of the constant
    ...
    def emitASTORE(self, in_, frame):
        ...
        # elif type(in_) is cgen.ArrayPointerType 
        elif type(in_) is ClassType or type(in_) is StringType:
    ...
    *   @param in the type of the operands.
    ...
    def emitINITARRAY(self, idx, size, idx_chain, in_, init, frame):
        if len(size) == 0:
            return ""
        res = ""
        for i in range(int(size[0])):
            # idx_chain.append(i)
            if len(size) == 1:
                res += self.jvm.emitALOAD(idx)
                frame.push()
                # for j in idx_chain[:-1]:
                res += self.emitPUSHICONST(i,frame)
                # res += self.emitPUSHICONST(idx_chain[-1],frame)
                res += init[i]
                # print(res)
                # res += self.emitPUSHCONST(init[idx_chain[-1]],in_,frame)
                res += self.emitASTORE(in_,frame)
            else:
                res += self.emitINITARRAY(idx,size[1:],idx_chain,in_,init[i],frame)
        return res

    def emitARRAY(self, idx, size, in_, init, frame):
        res = ""
        typ = ""
        for i in size:
            res += self.emitPUSHICONST(int(i),frame)
            typ += "["

        typ += self.getJVMType(in_)
        res += self.jvm.emitMULTIANEWARRAY(typ,str(len(size)))

        res += self.jvm.emitASTORE(idx)
        res += self.emitINITARRAY(idx, size, [], in_, init, frame)
    ...
    def emitNEGOP(self, in_, frame):
        ...
        if type(in_) is BoolType:
            return self.emitPUSHICONST(1,frame) + self.emitADDOP('-',BoolType(),frame) + self.jvm.emitINEG()
        else:
    ...
    *   generate ior
    ...
    def emitFRELOP(self, op, in_, trueLabel, falseLabel, frame):
        # op: String
        # in_: Type
        # trueLabel: Int
        # falseLabel: Int
    ...
    *   After:  ...,value1,value1<p>
    ...
    '''   generate code to exchange an floating-point number on top of stack to an integer.
    '''
    ...
    *   @param in the type of the returned expression.
    ...
    def emitRETURN(self, in_, frame):
        ...
        elif type(in_) is VoidType or in_ is None:
